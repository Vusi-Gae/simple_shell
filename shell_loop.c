 ...
 * @infor: Pointer to the parameter and returns info structure.
 * @_av: The argument vector from the main function.
 *
...
void find_cmd(info_t *infor)
{
	...
	for (j = 0, l = 0; infor->arg[j]; j++)
		if (!is_delim(infor->arg[j], " \t\n"))
			l++;
	if (!l)
		return;
